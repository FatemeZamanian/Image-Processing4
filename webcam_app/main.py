    ...
    def run(self):
        ...
        while True:
            ...
            for face in faces:
                ...
                frame[y:y + h, x:x + w] = resized_checkered
...
class Flip(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            self.set_flip_signal.emit(frame)
            cv2.waitKey(10)
    def stop(self):
        try:
            self.video.release()
...
class Mirror(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            self.set_mirror_signal.emit(frame)
            cv2.waitKey(10)
...
class Emoji(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            self.set_emoji_signal.emit(frame)
            cv2.waitKey(10)
...
class EyesMouth(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            faces = face_detector.detectMultiScale(frame, 3.1)
            for face in faces:
                x, y, w, h = face
                eyes = eye_detector.detectMultiScale(frame[y:y + h, x:x + w], 3.1)
                lips = lip_detector.detectMultiScale(frame[y:y + h, x:x + w], 3.3)
                for lip in lips:
                    xl, yl, wl, hl = lip
                    img_lip = frame[y +yl:y + yl + hl, x + xl:x + xl + wl]
                    ...
                    res = cv2.add(back, forg)
                    res *= 255
                    frame[y +yl:y + yl + hl, x + xl:x + xl + wl] = res

                for eye in eyes:
                    xe, ye, we, he = eye
                    img_eye = frame[y +ye:y + ye + he, x + xe:x + xe + we]
                    ...
                    res = cv2.add(back, forg)
                    res *= 255
                    frame[y + ye:y + ye + he, x + xe:x + xe + we] =res
                # result = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)

            self.set_em_signal.emit(frame)
            cv2.waitKey(10)
